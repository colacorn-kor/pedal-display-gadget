...
| 구분 | 회로 | 상태 |
|---|---|---|
| 현재 프로토타입 | TL072, 수동 LINE 2.00x / INST 7.82x, PCM1808 VINL 한 채널 | 오디오 구간 재배선 필요, 외부 9V 실측 전 |
| 제품 목표 | OPA2192 2개, HOT/SENSITIVE 동시 캡처, PCM1808 VINL/VINR 자동 선택 | 설계·펌웨어 준비, 부품 미보유·미실장 |

2026-08-03 사용자 확인 기준 OPA2192는 구입한 적이 없다. 따라서 목표 회로는 조달과
별도 승인 전까지 현재 조립 지시가 아니다. 기본 펌웨어도 `AUDIO_DUAL_RANGE=0`을 유지한다.
...
목표 회로를 만들기 전에 현재 회로의 기준값을 남긴다. 알려진 1kHz sine 신호원과
...
다음 조건이 모두 충족되기 전에는 목표 회로 실장이나 듀얼레인지 펌웨어 플래시를 하지 않는다.

- 현재 TL072 기준 측정이 기록돼 있다.
