...
> 기존 3화면(monitor/images/tuner) 마이그레이션 + `screen_manager` 디스패처 일반화.
> 입력 enum은 ①에서 의도적으로 미변경. **다음 = ②단계(입력 레이어: 입력 enum 교체,
> 6버튼+홈/풋스위치 숏롱, 오토리피트, GPIO 3개 추가).** 그다음 ③(런처 2줄·순서변경·퀵앱·
> 변형·설정). §3·§4는 ①의 실제 구현을 반영했고, §5~12는 미구현 설계(②③의 명세).
> 진척 상세는 §13 표 참조.
...
5. **매니저/앱 경계** — 매니저는 **풋스위치(숏/롱) + 홈**만 가로채고, 나머지 5키
   (상·하·좌·우·확인)는 전부 활성 앱에 위임.
...
typedef enum {
    ...
    EV_HOME, EV_HOME_HOLD,   /* 홈 숏 = 앱 위임 / 홈 롱 = 매니저 강제 탈출 */
...
| 홈 롱 | **강제 런처 복귀(안전장치)** | — | 순서변경 종료 → 런처 |
| 홈 숏 | **활성 앱이 해석**(나가기 등은 앱이 알아서) | 뒤로 (첫 화면이면 무동작) | 순서변경 종료 → 런처 |
| 상·하·좌·우·확인 | **전부 활성 앱이 자유 해석** | 커서 이동 / 확인=실행·진입 | 집어듦·이동·줄전환·내려놓음 |

> 홈 숏의 주인은 "지금 활성 앱이 있나"로 갈린다 — **라이브/오버레이 = 앱**, **런처 =
> 매니저(뒤로)**. 홈 롱(강제 탈출)은 오버레이까지 뚫고 런처로.

### 매니저가 가로채는 것 vs 위임하는 것

- **매니저 전담**: `EV_FOOTSW`, `EV_FOOTSW_HOLD`, `EV_HOME_HOLD`(강제 탈출).
- **활성 앱 위임**(라이브/오버레이 모드): `EV_UP/DOWN/LEFT/RIGHT/OK` + `EV_HOME`(숏)
  → `app->on_event()`. 앱이 나가기 경로(예: "나가시겠습니까?" 예/아니오)를 자체 구현.
- 런처/순서변경 모드에서는 활성 앱이 없으므로 5키 + 홈 숏을 매니저(런처 UI)가 사용.

→ 결과: 매니저는 **"앱 넘기기(풋스위치)"와 "탈출 안전장치(홈 롱)"** 두 책임만. 나머지
6키(상하좌우+확인+홈 숏)는 전부 앱. 핑퐁이든 고급 튜너든 자유롭게 쓰고, 앱이 먹통이어도
홈 롱으로 항상 빠져나온다.

### 하드웨어 영향

- 손버튼 6개 + 풋스위치 1개 = **입력 7핀**. 현재 손버튼 3핀(GPIO 4·5·6) + 풋스위치
  (GPIO 7) = 4핀. → **손버튼 3핀 추가**(6−3) 필요.
- 택트 ITS-1105 ×10 중 6개 사용(여유). **추가 구매 없음.**
- 추가 GPIO 3개는 스트래핑(0·3·45·46)·USB(19·20)·I2S(15·16·17·18)·SPI/디스플레이
  (8·9·10·11·12·14)·뮤트(21) 회피. 후보: GPIO 1, 2, 13, 38, 47, 48 중. **최종 핀은
  Codex가 DevKit 실크/핀맵 보고 확정.**
...
|----|------|-----------|------|------|
| `monitor` | Sound Monitor | SPECTRUM | — | `renderer_t`(곡선/막대/반응형) 중첩. 상하=테마/렌더러 |
| `tuner` | Tuner | TUNER | 기본/고급 | enter=뮤트. 고급=432/드롭/오프셋 |
| `images` | Images | SPECTRUM | — | 이미지·폴더 탐색(키 배정은 앱 자유) |
...
|---------------------------|---------|------|
| `SCR_MONITOR` + `select_monitor_renderer` | **`monitor` 앱.** `renderer_t`/테마 순환은 `on_event`로. vtable 그대로 중첩 | **①완료** (현재 PREV=렌더러, NEXT=테마. 방향키 재배치는 ②) |
| `SCR_IMAGES` + 이미지 순환 | **`images` 앱.** `on_event`로 이미지 전환 | **①완료** (PREV/NEXT 순환) |
| `SCR_TUNER` + 뮤트 특별취급 | **`tuner` 앱.** enter=뮤트, audio=TUNER, variant=2 | **①완료** (뮤트/모드 자기소유) |
| `SCR_HOME` 메뉴 | **런처**로 역할 변경(단순 메뉴 → 2줄 체인 + 활성화/순서/설정) | ①=레지스트리 생성 단순 메뉴까지. **2줄/순서/설정은 ③** |
| enum `screen_t` + 거대 switch | **활성 앱 인덱스 + 디스패치** (→ ③에서 `s_slots[]` + 모드) | ①=활성 인덱스+홈/앱 디스패치. **slots/모드는 ③** |
| `sm_on_event`의 화면별 분기 | 매니저는 풋스위치/홈만, 나머지는 활성 앱 `on_event` 디스패치 | **①완료** ("앱 우선, 매니저 폴백" 골격) |
| `ui_event_t {PREV,NEXT,SELECT,BACK,FOOTSW}` | `{UP,DOWN,LEFT,RIGHT,OK,HOME,HOME_HOLD,FOOTSW,FOOTSW_HOLD}` | **①에선 미변경(의도)** — 입력 enum 교체는 **②**. ①은 구 enum 위에서 동작 |

**①에서 실제 생성/변경된 파일:** `gadget_app.h`·`gadget_app.c`(인터페이스+레지스트리),
`app_monitor.c`·`app_images.c`·`app_tuner.c`(3앱), `screen_manager.c`(디스패처로 일반화),
`main/CMakeLists.txt`(빌드 등록). 풋스위치는 ①에서 **현행 동작 재현 임시 브릿지**로 유지
(②에서 숏/롱 정책으로 교체). `sm_load_scene*`은 완전 재배선(앱 상태 경유).

**①에서 의도적으로 안 한 것(=②③ 몫):** 입력 enum 교체·6버튼·홈/풋스위치 숏롱·오토리피트
(②), 2줄 체인·순서변경·퀵앱 오버레이·변형 화면 분기·설정 영속성(③).

**보존되는 것(①에서 지켜짐):** Core1 오디오/seqlock, `renderer_t` 전체, LVGL lock 규약,
...
- 추가 손버튼 **GPIO 3개** 최종 핀 번호(DevKit 핀맵 기준).
- "퀵 앱" 뱃지의 시각 표현(화면 구석 작은 인디케이터).
- 런처 2줄 레이아웃의 구체 픽셀(LVGL).
- `on_event` 오토리피트 간격(ms)과 디바운스 파라미터.
...
- [ ] 앱 전환 = 풋스위치 전용, 상·하·좌·우·확인 + **홈 숏**은 앱 자유 해석
- [ ] 매니저 가로채기 = 풋스위치(숏/롱) + **홈 롱(강제 탈출)**
- [ ] 두 체인 = 한 배열 두 뷰, 활성화 = 줄 옮기기
- [ ] 퀵 앱 = 직교 오버레이 + 뱃지 + 상태 스냅샷 복귀
- [ ] 6버튼(상하좌우+홈+확인), 방향키 오토리피트, 롱 감지 = 풋스위치·홈 둘만
- [ ] 신성한 원칙 = 출력 enum에 OUT_MAIN 부재
- [ ] Phase 2 앱은 `requires_codec`로 비활성, 라우팅 필드는 예약
