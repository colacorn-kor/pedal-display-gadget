...
| 게임 콘텐츠 | PC 폴더 ROM | MicroSD ROM | 같은 카탈로그와 코어 어댑터 |
...
| Gallery | PC 폴더 이미지 표시 | SD 소프트웨어 구현, 실기 대기 | 브라우징·오류·메타데이터 개선 |
| Bounce | 동작 | 동작 | 공통 게임 루프 유지 |
| Metronome | 미구현 | 미구현 | PC 출력과 UI부터 구현 |
| Music | 파일 카탈로그만 존재 | 파일 카탈로그만 존재 | PC 재생부터 구현, ESP 코덱 이식 |
| Retro | ROM 카탈로그만 존재 | ROM 카탈로그만 존재 | 지원 코어를 명시하고 실제 실행 구현 |
...
- Retro는 GG2 범위인 GBA/NDS를 포함하지 않는다. GG에서 감당할 Retro-Go급 코어만
